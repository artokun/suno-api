...
import { Page, Locator, chromium, firefox, BrowserContext } from "playwright-core";
...
class SunoApi {
  ...
  */
  ...
  private async launchBrowser(): Promise<BrowserContext> {
    ...
    if (!isDev) {
      args.push(...chromiumAws.args, "--disable-gpu");
    }
    ...
    const browser = await this.getBrowserType().launch({
      args,
      executablePath: isDev ? undefined : await chromiumAws.executablePath(),
      headless: yn(process.env.BROWSER_HEADLESS, { default: true }),
      chromiumSandbox: false
    });
    const userAgent = isDev ? this.userAgent : 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
    const context = await browser.newContext({ userAgent, locale: process.env.BROWSER_LOCALE, viewport: null });
    const cookies = [];
